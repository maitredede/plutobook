...
# ---------------------------------------------------------------------------
# Hash du commit de correctif par finding (rempli quand le fix est livre).
FIXCOMMITS = {
    ...
    "V17": "524e7f7", "V18": "e4809ac", "V19": "aa01f02", "V20": "5e2f11e",
    "V21": "e655c98", "V22": "f8e4389",
}

F = []
def add(**k):
    # status: "todo" (a corriger) ou "done" (corrige). fixcommit: hash du commit.
...
add(
 ...
 locations=[
   ("source/layout/tablebox.cpp", "TableSectionBox::layout() / layoutRows(): layout complet de chaque cellule, deux fois par niveau"),
   ("source/layout/tablebox.cpp", "TableBox::build() / exceedsMaxNestingDepth(): profondeur d'imbrication et coupe-circuit"),
   ("include/plutobook.hpp", "EngineLimits::maxTableNestingDepth"),
 ],
 nature="""<p>Diagnostic confirme par comptage d'appels (instrumentation temporaire) : le cache de largeur
 preferee min/max par boite (<code>BoxFrame::m_minPreferredWidth</code>/<code>m_maxPreferredWidth</code>,
 invalide seulement si &lt; 0) fonctionne correctement -- <code>minPreferredWidth()</code>/<code>maxPreferredWidth()</code>
 ne sont calcules qu'une fois par boite, cout <code>O(N)</code> mesure pour <code>N</code> tables
 imbriquees. Le veritable doublement vient du <strong>layout</strong> (pas du calcul de largeur) : l'algorithme
 de hauteur de table CSS 2.1 est intrinsequement a deux passes par table --
 <code>TableSectionBox::layout()</code> relaie d'abord chaque cellule a sa hauteur naturelle (non etiree)
 pour mesurer la hauteur de chaque ligne, puis <code>TableSectionBox::layoutRows()</code> relaie
 <em>a nouveau</em> chaque cellule avec cette hauteur de ligne appliquee en override, pour etirer les
 cellules a la ligne (necessaire pour <code>vertical-align</code> et les descendants en hauteur
 pourcentage). Les deux passes relaient integralement le contenu de la cellule, y compris une table
 imbriquee a l'interieur : une table imbriquee sur <code>N</code> niveaux est donc laid out ~<code>2^N</code>
 fois, pas <code>N</code> fois -- mesure : 62, 2046, 65534 appels de layout de cellule pour N=5, 10, 15
 (formule exacte <code>2^(N+1)-2</code>), independant du cap de profondeur d'imbrication generale de V08
 (512).</p>""",
 risk="<p><strong>Impact</strong> : hang CPU (DoS) sous le cap de profondeur general.</p>",
 repro=["Generer poc/make-nested-tables.py (~150 niveaux) &rarr; rendre &rarr; hang pre-fix (&gt;20s), ~0.9s post-fix."],
 poc={
...
""",
 ...
 fix="""<p>La memoisation seule ne corrige pas le probleme (le calcul de largeur est deja mis en cache et
 lineaire ; le cout exponentiel vient du <em>layout</em>, pas du calcul de largeur, et memoiser un layout
 complet -- positions, overflow, fragmentation -- serait bien plus invasif et risque qu'une largeur).
 Correctif choisi (repli explicitement documente dans ce dossier comme alternative acceptable) : borner
 specifiquement la profondeur d'imbrication des <em>tables</em> (distincte du cap general de 512).
 <code>TableBox</code> connait desormais sa profondeur parmi ses ancetres tables
 (<code>m_nestingDepth</code>, derivee dans <code>build()</code> -- O(1) amorti, l'ancetre le plus proche a
 deja sa propre profondeur calculee puisque <code>build()</code> parcourt l'arbre du haut vers le bas).
 Au-dela de <code>EngineLimits::maxTableNestingDepth()</code>, <code>TableSectionBox::layoutRows()</code>
 saute la seconde passe (etirement) pour les cellules de cette table : la cellule garde la hauteur
 naturelle mesuree par la premiere passe -- meme contenu, meme largeur, meme position, simplement pas
 etiree si une cellule voisine de la meme ligne est plus haute. Ce doublement recursif fait que le cout
 total est <code>O(2^limite &times; profondeur_totale)</code> et pas seulement <code>O(2^limite)</code>
 (chaque niveau au-dela de la limite dedouble a nouveau tout ce qui est imbrique au-dessus) : la limite
 doit donc rester basse pour que <code>2^limite</code> reste une petite constante, contrairement aux
 autres limites de ce tableau ou une valeur de l'ordre de 100 suffirait.</p>""",
 config="""Nouvelle limite <code>EngineLimits::maxTableNestingDepth</code>
 (<code>setMaxTableNestingDepth</code>/<code>maxTableNestingDepth()</code>), <strong>defaut 8</strong>,
 <code>0</code> = illimite (non recommande, cout reellement exponentiel sans elle). API C
 <code>plutobook_set_max_table_nesting_depth(unsigned int)</code>. Verifie : mise a l'echelle desormais
 lineaire (0.19s/0.63s/0.89s/0.98s pour 50/100/150/200 niveaux, contre hang/timeout pre-fix des N&asymp;20-25) ;
 PoC 150 niveaux -&gt; ~0.92s, PDF valide (pre-fix : timeout &gt;20s). Non-regression verifiee par
 <strong>comparaison PDF octet-par-octet</strong> entre binaire pre-fix (<code>git stash</code> + build
 separe) et post-fix sur : grille simple, colspan/rowspan, imbrication a 2 niveaux, largeurs %/fixe/auto,
 caption (les 5 -- identiques au byte pres) ; imbrication reelle jusqu'a exactement la limite (profondeur
 8, incluse) -- identique au byte pres ; au-dela de la limite (profondeur 9+), differences observees
 uniquement quand une ligne contient des cellules de hauteurs naturelles differentes (la plus courte
 n'est plus etiree) -- comportement exactement documente, pas de corruption.""",
 status="done",
)
...
add(
 ...
 locations=[
   ...
   ("source/graphics/textshape.cpp", "TextShapeView::width/maxAscentAndDescent/draw/expansionOpportunityCount rescannent aussi tous les runs depuis le glyphe 0"),
 ...
 pagine (independant du multicolonne).</p>
 <p><strong>Diagnostic affine pendant le correctif</strong> : corriger uniquement ces deux fonctions
 (recherche dichotomique au lieu du rescan) n'a <em>pas</em> suffi -- mesure empirique a l'appui
 (5000/10000/20000/40000 mots : 0,8s / 2,9s / 11,4s / 42s, toujours quadratique). Instrumentation par
 compteurs d'iterations : le veritable morceau dominant etait dans <code>TextShapeView::width()</code>,
 <code>maxAscentAndDescent()</code> et <code>draw()</code> (et par le meme motif,
 <code>expansionOpportunityCount()</code>) -- ces methodes parcourent <em>tous les runs de la shape</em>
 a chaque appel et, pour tout run entierement hors de la plage [startOffset,endOffset) de la vue courante,
 balaient neanmoins la totalite de ses glyphes sans le moindre effet (aucune condition ne devient vraie),
 uniquement pour le "sauter". Meme situation qu'un run unique assez long : la vue d'une ligne tardive
 revisite tout ce qui precede a chaque appel. Une fois ces quatre methodes corrigees en plus des deux
 nommees dans le titre, la mise a l'echelle mesuree redevient quasi lineaire (voir Correctif).</p>""",
 risk="""<p><strong>Impact</strong> : hang/lenteur CPU (DoS) a partir d'un long document texte
 (multicolonne ou non). Cout ~4-5x par ligne en fin de document vs debut, mesure ; sans le correctif
 complet (les 4 methodes de <code>TextShapeView</code> incluses), le PoC 200 000 mots reste hors delai
 (&gt;40s) meme avec la recherche dichotomique sur les deux seules fonctions nommees dans ce finding.</p>""",
 repro=[
   "Generer poc/make-longtext.py (long texte simple colonne), rendre : superlineaire pre-fix (40s+ timeout), ~lineaire post-fix (1,4s).",
   "Le PoC multicolonne de V21 devient egalement rapide apres ce correctif (60s timeout pre-fix -> 0,24s post-fix, mesure sur poc/make-multicol.py de V21).",
 ],
 poc={
...
""",
 ...
 fix="""<p>Recherche dichotomique sur les offsets de glyphes (monotones) au lieu d'un rescan depuis 0 :
 <code>TextShapeRun::positionForOffset()</code>/<code>offsetForPosition()</code> deviennent O(log n) par
 appel via une nouvelle methode privee <code>glyphIndexForOffset()</code> et une somme cumulee des
 avances (<code>advanceUpTo</code>) precalculee une seule fois a la creation du run (O(n), amorti sur
 tous les appels ulterieurs). <code>positionForOffset</code> recherche sur <code>characterIndex</code>,
 monotone par construction HarfBuzz (croissant pour LTR, decroissant pour RTL) quel que soit le signe
 des avances -- toujours sur. <code>offsetForPosition</code> recherche sur la somme cumulee elle-meme,
 qui n'est monotone que si toutes les avances sont &ge; 0 (le cas normal) ; un <code>letter-spacing</code>/
 <code>word-spacing</code> negatif suffisamment fort peut rendre une avance negative -- dans ce cas
 (rare, detecte une fois par run a la creation), <code>offsetForPosition</code> retombe sur le rescan
 lineaire original verbatim, pour garantir un resultat identique en toute circonstance plutot que de
 supposer une monotonie non garantie.</p>
 <p>Complement necessaire (voir Nature) : <code>TextShapeView::width()</code>,
 <code>maxAscentAndDescent()</code>, <code>draw()</code> et <code>expansionOpportunityCount()</code>
 corrigees du meme mal avec la meme technique -- une nouvelle fonction <code>rangeForView()</code>
 saute en O(1) tout run entierement hors de la vue (et arrete la boucle externe des que les runs
 restants le sont necessairement aussi, l'ordre des runs etant croissant en LTR / decroissant en RTL)
 et utilise <code>glyphIndexForOffset()</code> pour sauter directement au premier glyphe pertinent du
 run qui chevauche la vue, au lieu de rescanner depuis 0. Cas particulier de <code>draw()</code> : elle a
 un effet de bord visible par run (appels cairo), donc contrairement aux trois autres (pures, sans effet
 pour un run saute), sa boucle externe continue de visiter <em>tous</em> les runs dans le meme ordre et
 emet les memes appels cairo pour chacun (seul le balayage de glyphes est raccourci/supprime, ce qui
 laisse <code>numGlyphs</code> a 0 exactement comme le balayage original l'aurait fait pour un tel run).</p>""",
 config="""Correctif algorithmique (pas de knob ; la longueur de noeud texte est deja bornee en V19).
 Verification -- <strong>mise a l'echelle</strong> : poc/make-longtext.py (200k mots) 40s+ timeout &rarr;
 1,4s ; a 2x (400k mots) 3,7s, a 4x (800k mots) 11,0s (legerement superlineaire au-dela de la correction
 textshape.cpp elle-meme -- confirme par compteurs d'iterations que les methodes <code>TextShapeView</code>
 et <code>TextShapeRun</code> corrigees scalent, elles, parfaitement lineairement avec la taille du
 document ; le residu vient d'ailleurs dans le pipeline, hors perimetre) ;
 5000/10000/20000/40000 mots passent de 0,8s/2,9s/11,4s/42s (quadratique, correctif partiel) a
 0,075s/0,098s/0,15s/0,27s (quasi lineaire, correctif complet). <strong>PoC V21</strong>
 (poc/make-multicol.py, 20000 mots / 3 colonnes) : 60s timeout pre-fix (binaire via
 <code>git stash</code> + build separe) &rarr; 0,24s post-fix. <strong>Non-regression, identite au byte
 pres</strong> (binaire pre-fix via <code>git stash</code> + build meson separe, compare au binaire
 post-fix) sur 14 documents : LTR multi-paragraphe, texte justifie (expansionOpportunityCount),
 polices/styles/tailles melanges dans une meme ligne, mots a ligatures (ffi/ffl/fi/fl),
 <code>letter-spacing</code> negatif (y compris chemin de repli), arabe RTL (Noto Sans Arabic), hebreu
 RTL (Noto Serif Hebrew), paragraphe bidi LTR/RTL/RTL melange dans la meme ligne, tabulations, cas
 limites (paragraphe vide/un caractere/un espace), texte SVG (chemin <code>TextShapeView::draw</code>
 distinct de linelayout.cpp), longtext reduit, PoC V21 multicolonne reduit -- toutes les sorties PDF
 identiques au byte pres (<code>cmp</code>) ; 6 de ces documents egalement identiques au byte pres en
 sortie PNG (<code>html2png</code>).""",
 status="done",
)
